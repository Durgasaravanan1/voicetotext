  ...
  return (
    <div className="min-h-screen bg-white flex justify-center p-5 font-mono">
      <div className="w-full max-w-[1200px]">
        ...
        <div className="bg-cyan-400 rounded-t-[30px] px-6 py-5 flex justify-between">
          <div className="text-xl font-bold">🩺 NOTEWHISPER — DOCTOR MODE</div>
          <div className="bg-white border-4 border-black px-6 py-2 font-bold">
            ACTIVE
          </div>
        </div>
        ...
        <div className="border-4 border-cyan-400 border-t-0 rounded-b-[30px] p-8">

          <div className="border-4 border-black px-6 py-4 mb-8">
            PATIENT_RECORDS
          </div>
          ...
          <div className="space-y-6">
            {patients.map((p, i) => (
              <div
                key={i}
                onClick={() => navigate(`/patient/${p.patient_id}`)}
                className="border-4 border-black p-6 cursor-pointer hover:bg-gray-100"
              >
                <div><b>ID:</b> {p.patient_id}</div>
                <div><b>Symptoms:</b> {p.symptoms || "-"}</div>
                <div><b>Medicines:</b> {p.medicines || "-"}</div>
                <div><b>Follow-up:</b> {p.follow_up_required ? "YES" : "NO"}</div>
              </div>
            ))}
          </div>
